...
def draw_graph(graph):
    ...
    node_colors = [get_color(graph.nodes[node].get('label', 'TOKEN')) for node in graph]
    node_labels = {node: f"{node} ({graph.nodes[node].get('label', 'TOKEN')})" for node in graph}

    nx.draw(graph, pos, with_labels=False, node_color=node_colors, edge_color='gray', node_size=500, font_size=8, font_weight='bold', arrows=True)  # Ajusta node_size y font_size según sea necesario
    nx.draw_networkx_labels(graph, pos, labels=node_labels, font_size=8, horizontalalignment='left')
