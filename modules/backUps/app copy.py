...
with tab1:
    ...
    with subtab2:
        ...
        #with col2:
        #    st.markdown(f"""
        #    ### Leyenda de Categorías Gramaticales
        #    <div style='display: flex; flex-wrap: wrap;'>
        #        {"".join([f"<div style='margin-right: 20px;'><strong style='color: {COLOR_MAP[label]};'>{label}:</strong> {POS_LABELS[label]}</div>" for label in POS_LABELS])}
        #    </div>
    ...
    with subtab4:
        ...
        # Sección para el texto patrón y su gráfico
        with st.container():
        ...
        # Sección para el texto a comparar y su gráfico
        with st.container():
            col1, col2 = st.columns([2, 3])
            with col1:
